...
module.exports = {
    ...
    async execute(message, args, cmd, client, Discord, profileData){
        ...
        const error = (str) => message.channel.send(new Discord.MessageEmbed().setColor('RED').setTitle('PyHe Bot ||Hata').setDescription(str));
    ...
    try {
        switch (option) {
                        ...
                        switch (reaction.emoji.name) {
                            case '✅':
                                ...
                                const islemEmbed = new Discord.MessageEmbed()
                                    .setTitle('PyHe Bot || Toplu Rol Silme')
            ...
            case 'ver':
                            ...
                            case '✅':
                                ...
                                addToFilter.forEach((member) => {
                                    const timeout = setTimeout(() => {
                                        ...
                                        m.edit(new Discord.MessageEmbed().setTitle('PyHe Bot || Toplu Rol Verme').setColor('GREEN').addField('Toplam Kaç Kişiye Verilecek', addToCount, true).addField('Kaç Kişi Kaldı', kalanKisi).addField('Yüzdelik', `İşlem Yapılan: %${yuzdeHesapla(kalanKisi, addToFilter.size)}`).setFooter(`yaklaşık ${yaklasikSure(kalanKisi)} süre kaldı`))
                                        clearTimeout(timeout)
                                        kalanKisi = kalanKisi - 1;
                                        if (kalanKisi == 0) {
                ...
                break;
            default:
                error(`Lütfen bir opsiyon belirtin.\n\n\`• !rol ver <hangiRoldekiÜyelereVerilecek> <verilecekRol>\`\n\`• !rol al <hangiRoldekiÜyelerdenAlınacak> <alınacakRol>\``);
        }
    } catch (err) {
